...
- [ ] Починить сборку `services/api` (типы, импорты, DTO, зависимости).
- [ ] Привести `User.id`/FK к единому типу (number vs string vs bigint).
- [ ] Запустить минимальный набор модулей:
  - [ ] `auth`
  - [ ] `user`
  - [ ] `game` (hunt + action + state + respawn)
- [ ] Отключить/фичефлагом убрать второстепенные модули до стабилизации (`payment`, часть `notification`, часть `referral`).
...
- [ ] Подключить фронт к живым endpoint-ам MVP.
- [ ] Проверить состояние пользователя, бой, награды, воскрешение.
- [ ] Подправить UX и обработку ошибок (401, сеть, timeouts).
- [ ] Добавить smoke-проверки для ключевых сценариев.
